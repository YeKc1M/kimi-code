...
Only **blockable events** (`PreToolUse`, `Stop`, `UserPromptSubmit`) have return values that affect the main flow. All other events are **observation-only events**: they fire and forget, and the main flow is unaffected regardless of what the script returns. One exception: with the experimental `hook_permission_decisions` flag enabled, [`PermissionRequest`](#deciding-approvals-from-a-hook-experimental) hooks can approve or deny the pending approval request.
:::
